function [R, pValues] = bst_corrn_g(X, Y, RemoveMean)
...
% note that the envelope is just the magnitude, NOT the power.
% Note that RemoveMean is necessary (otherwise the result of the matrix
% prodcut in line 78 is not a correlation, but a scalar product). 
% Check a comparison corrcoef(Xc(1,:), Xc(2,:))


for iX=1:size(X,1)
    X(iX,:)=abs(hilbert(X(iX,:)));
end;

for iY=1:size(Y,1)
    Y(iY,:)=abs(hilbert(Y(iY,:)));
end;
