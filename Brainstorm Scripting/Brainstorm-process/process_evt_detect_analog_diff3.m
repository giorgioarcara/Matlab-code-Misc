function varargout = process_evt_detect_analog_diff2( varargin )
% PROCESS_EVT_DETECT_ANALOG: Detect the trigger on an analog channel
%
% USAGE:  OutputFiles = process_evt_detect_analog_diff2('Run', sProcess, sInputs)
%                 evt = process_evt_detect_analog_diff2('Compute', F, TimeVector, OPTIONS, Fmask)
%                 evt = process_evt_detect_analog_diff2('Compute', F, TimeVector, OPTIONS)
%             OPTIONS = process_evt_detect_analog_diff2('Compute')                                : Get the default options structure
...
% =============================================================================@
%
% Original Authors: Elizabeth Bock, Francois Tadel, 2013-2014
% Modified by: Marco D'Alonzo, Giorgio Arcara, 2018
...
sProcess.Comment     = 'Detect analog triggers with diff (2)';
...
sProcess.options.expl.Comment = 'Detect trigger via differential ( diff() function) <BR> (i.e., with approximate derivative)';
...
sProcess.options.blanking.Value   = {1.5, 's', []};
% Event Duration
sProcess.options.EventDur.Comment = 'Minimum Duration of Event: ';
sProcess.options.EventDur.Type    = 'value';
sProcess.options.EventDur.Value   = {0.3, 's', 2};
% Event Duration
sProcess.options.MinInt.Comment = 'Minimum Interval to find event';
sProcess.options.MinInt.Type    = 'value';
sProcess.options.MinInt.Value   = {0.1, 's', 2};
% Smoothing
sProcess.options.Smooth.Comment = 'Smoothing Parameter';
sProcess.options.Smooth.Type    = 'value';
sProcess.options.Smooth.Value   = {10, 'value', 0};
% Proportion
sProcess.options.Prop.Comment = 'Proportion to detect Events';
sProcess.options.Prop.Type    = 'value';
sProcess.options.Prop.Value   = {0.95, 'value', 2};
% Separator
...
OPTIONS.EventDur = sProcess.options.EventDur.Value{1};
OPTIONS.MinInt = sProcess.options.MinInt.Value{1};
OPTIONS.Smooth = sProcess.options.Smooth.Value{1};
OPTIONS.Prop = sProcess.options.Prop.Value{1};
...
%% GIORGIO, from the original process_detect_detect_analog
% I deleted everything from line 364 to 489

% SECTION by MARCO D'ALONZO

window = OPTIONS.blanking;
interval = OPTIONS.EventDur;
min_int = OPTIONS.MinInt;
Smo = OPTIONS.Smooth;
Prop = OPTIONS.Prop;

data = F;
data_s = smooth(data, Smo);

% window = 1.5 % distanza minima tra i punti dello stesso tipo
% interval = 0.3 %lunghezza minima dell'evento
% min_int = 0.1 % parametro di controllo per valutare la ripidit? del cambiamento d'evento


window_samp = round(window*sFreq);
interval_samp = round(interval*sFreq);
min_int_samp = round(min_int*sFreq);
ind_pos = find(data_s > 0);
dif_ind_pos = diff(ind_pos);
red_ = ind_pos(dif_ind_pos > interval_samp);
ind_neg = find(data_s < 0);
dif_ind_neg = diff(ind_neg);


green_ = ind_neg(dif_ind_neg > interval_samp);
red_ = red_(find(diff(red_)> window_samp)+1);
green_ = green_(find(diff(green_)> window_samp)+1);

% "red" is a coding for triggers after descending modification in signal
% "green" is a coding for triggers after rising modification signal

if (OPTIONS.isfalling)
    evt = {TimeVector(red_)};  
    j = 1;
    for i = 1:length(red_)
        if ~((red_(i) < window_samp) || ((length(data_s) - red_(i)) < window_samp))
            mx_data = max(data_s(red_(i) + (-window_samp:window_samp)));
            last_bef_r = find(data_s(red_(i) + (-min_int_samp:0)) > Prop*mx_data,1,'last');% valutazione della ripidita
            if (~isempty(last_bef_r))
                red_(j) = red_(i) - min_int_samp + last_bef_r;
                j = j+1;
            end
        end
    end
    ...
    j = 1;
    for i = 1:length(green_)
        if ~((green_(i) < window_samp) || ((length(data_s) - green_(i)) < window_samp))
            mn_data = min(data_s(green_(i)  + (-window_samp:window_samp)));
            last_bef_g = find(data_s(green_(i) + (-min_int_samp:0)) < Prop*mn_data,1,'last');
            if (~isempty(last_bef_g))
                green_(j) = green_(i) - min_int_samp + last_bef_g;
                j = j+1;
            end
        end
    end
