...
function sProcess = GetDescription() %#ok<DEFNU>
    ...
    sProcess.Comment     = 'Detect bad channels: absolute';
    ...
    sProcess.options.meggrad.Type    = 'value';
    sProcess.options.meggrad.Value   = {0, 'fT', 2};  % Factor 15
    % MEG MAG
    sProcess.options.megmag.Comment = str_pad('MEG magneto:');
    sProcess.options.megmag.Type    = 'value';
    sProcess.options.megmag.Value   = {0, 'fT', 2};   % Factor 15
    % EEG
    sProcess.options.eeg.Comment = str_pad('EEG:');
    sProcess.options.eeg.Type    = 'value';
    sProcess.options.eeg.Value   = {0, 'muV', 2};   % Factor 6
    % SEEG/ECOG
    sProcess.options.ieeg.Comment = str_pad('SEEG/ECOG:');
    sProcess.options.ieeg.Type    = 'value';
    sProcess.options.ieeg.Value   = {0, 'muV', 2};   % Factor 6
    % EOG
    sProcess.options.eog.Comment = str_pad('EOG:');
    sProcess.options.eog.Type    = 'value';
    sProcess.options.eog.Value   = {0, 'muV', 2};   % Factor 6
    % ECG
    sProcess.options.ecg.Comment = str_pad('ECG:');
    sProcess.options.ecg.Type    = 'value';
    sProcess.options.ecg.Value   = {0, 'mV', 2};   % Factor 3
    % Explanations
    sProcess.options.comment1.Comment = ['<BR>  - <B>First column</B>: Signal detection threshold (abs)<BR>' 10]; %GIORGIO ...
...
function Comment = FormatComment(sProcess)
    ...
    if (sProcess.options.rejectmode.Value == 1)
        Comment = 'Detect bad channels: abs'; % GIORGIO Peak-to-peak ';
    else
        Comment = 'Detect bad trials:  abs'; % GIORGIO Peak-to-peak ';
    end
    % What are the criteria
    for critName = {'meggrad', 'megmag', 'eeg', 'eog', 'ecg'}
        if ~isequal(sProcess.options.(critName{1}).Value{1}, 0); % GIORGIO [0, 0])
...
function OutputFiles = Run(sProcess, sInputs) %#ok<DEFNU>
    ...
    for iFile = 1:length(sInputs)
        ...
        for iMod = 1:length(Modalities)
            ...
            % BEGIN GIORGIO. NOTE!  I leave p2p to modify as little as possible
            % the code
            maxp = max(abs(DataToTest),[], 2); % GIORGIO
            % Get bad channels
            % iBadChanMod = find((p2p < Threshold(1)) | (p2p > Threshold(2)));
            iBadChanMod = find(maxp > Threshold(1));
