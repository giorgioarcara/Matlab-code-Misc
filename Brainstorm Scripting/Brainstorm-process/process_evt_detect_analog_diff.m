function varargout = process_evt_detect_analog_diff( varargin )
% PROCESS_EVT_DETECT_ANALOG: Detect the trigger on an analog channel
%
% USAGE:  OutputFiles = process_evt_detect_analog_diff('Run', sProcess, sInputs)
%                 evt = process_evt_detect_analog_diff('Compute', F, TimeVector, OPTIONS, Fmask)
%                 evt = process_evt_detect_analog_diff('Compute', F, TimeVector, OPTIONS)
...
function sProcess = GetDescription() %#ok<DEFNU>
    ...
    sProcess.Comment     = 'Detect analog triggers with diff';
    ...
    % Threshold
    sProcess.options.threshold.Comment = 'Amplitude diff threshold: ';
    sProcess.options.threshold.Type    = 'value';
    sProcess.options.threshold.Value   = {-2e-4, 'value', 6};
    % Blanking period
...
function evt = Compute(F, TimeVector, EventSamps, OPTIONS, Fmask)
    ...
    threshVal = OPTIONS.threshold;

    if OPTIONS.isfalling
        % Find all the indices that are above the threshold
        %iThresh = find(Fabs(1:end-blankSmp) < threshVal);
        iThresh = find(diff(F(1:end-blankSmp)) < threshVal); % Giorgio -2e-4
        
        
    else
        % Find all the indices that are above the threshold
        % iThresh = find(Fabs(1:end-blankSmp) > threshVal);
        iThresh = find(diff(F(1:end-blankSmp)) > threshVal); % Giorgio
    end
