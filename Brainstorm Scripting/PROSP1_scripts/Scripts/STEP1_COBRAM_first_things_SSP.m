...
% 3) Add an end bad segment to epoched files if necessary (only in PROSP1 objects).
...
my_sFiles = sel_files_bst({my_sFiles_ini.FileName}, 'PROSP1S001|COB02|COB03|COB04|COB05|COB06|COB07|COB08|COB09|COB10');
...
% Input files
epoched_files_bad=sel_files_bst(my_sFiles, 'PROSP1');
...
Subj_files = sel_files_bst(my_sFiles, 'Resting|PROSP1');
...
%% PROSP1 FILES - adjust trigger
PROSP1_files = sel_files_bst({Subj_files.FileName}, 'PROSP1');


bst_report('Start', PROSP1_files);


PROSP1_files = bst_process('CallProcess', 'process_evt_detect_analog', PROSP1_files, [], ...
    'eventname',   'Met_adj', ...
    'channelname', 'UADC001', ...
    'timewindow',  [,], ...
    'threshold',   2, ...
...
% Save and display report
ReportFile = bst_report('Save', PROSP1_files);
bst_report('Open', ReportFile);
bst_report('Export', ReportFile, [export_main_folder, '/', export_folder]);
