function OutputFiles = bst_connectivity_g(FilesA, FilesB, OPTIONS)
...
% MODIFICATION: GIORGIO ARCARA, line 277 point to bst_corrn_g, 
% line 276 "Envelope corr" in the comment
% lines 684, 686, envelcorrn in the file name
...
function NewFile = SaveFile(R, iOutputStudy, DataFile, sInputA, sInputB, Comment, nAvg, OPTIONS, FreqBands)
    ...
    if OPTIONS.isSave
        ...
        if (length(FileMat.RefRowNames) == 1)
            fileTag = 'connect1_envel';
        else
            fileTag = 'connectn_envel'; % NOTE it is importnat to perserve connectn. It is parsedo to determin the file type
        end
        % Output filename
