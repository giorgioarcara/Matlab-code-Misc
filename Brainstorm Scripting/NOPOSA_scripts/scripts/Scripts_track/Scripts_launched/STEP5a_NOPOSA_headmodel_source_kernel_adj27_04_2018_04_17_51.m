...
my_sFiles = sel_files_bst({my_sFiles_ini.FileName}, 'standard_adj|bl|Sound_adj');


%% LOAD FILES TO BE MODIFIED
run_to_mod = readtable('/DATA/NFS_DATI_ORIG/VServer22/MEG/NOPOSA/NP_Check_view_reports.txt', 'ReadVariableNames', false);
run_to_mod = table2cell(run_to_mod);

% create a string that will select only the files we want to calculate
% again
check = run_to_mod{1};
for iRun= 2:length(run_to_mod)
    check=[check, '|', run_to_mod{iRun}];
end;


my_sFiles = sel_files_bst(my_sFiles, check);
