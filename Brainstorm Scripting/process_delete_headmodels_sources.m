...
for iStudy = 1:length(sStudies) 
    ...
    % exlude links, that would cause problems
    % (cause after file_fullpath, the link become the kernel)
    exclude_string = 'link';
    excl_links=regexp(all_files, exclude_string);
    incl_links_indices=find(cellfun(@isempty, excl_links));
    
    all_files_sel = all_files(incl_links_indices);
    all_files_sel = file_fullpath(all_files_sel);
      
    
    cellfun(@delete, all_files_sel);
